...
public abstract class BaseListActivity extends ListActivity implements
	...
	public void setupList() {
		...
		if (mAdapter == null) {
			mAdapter = new SimpleCursorAdapter(this, getListLayout(),
					getCursor(), getFrom(), getTo());
		}
	...
	protected Cursor getCursor() {
		return managedQuery(getUri(), getProjectionArray(), getSelection(),
	...
	protected void showDeleteDialog(final Runnable deleteAction) {
		//This dialog doesn't persist through rotations.
	...
	protected void addContextMenuItems(ContextMenu menu,
			AdapterContextMenuInfo info, long id) {
	...
	protected boolean handleContextMenuSelection(Intent intent,
			final long itemId) {
		if (intent.getAction().equals(Intent.ACTION_EDIT)) {
			...
			showDeleteDialog(new Runnable() {
				@Override
				public void run() {
					getContentResolver().delete(getUri(), Dao._ID + " = ?",
